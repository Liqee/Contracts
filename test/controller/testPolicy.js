...
describe("Controller: Policy Hooks", function () {
  ...
  describe("beforeRedeem()/afterRedeem()", function () {
    ...
    it("Should revert in beforeRedeem() if token price is unavailable in controllerStock", async function () {
      const {
        controllerStock,
        iToken0,
        mockPriceOracle,
      ...
      // Use controllerStock
      await iToken0._setController(controllerStock.address);
      ...
      await iToken0.connect(other).mint(account, amount);
      await controllerStock.connect(other).enterMarkets([iToken0.address]);
  ...
  describe("beforeBorrow()/afterBorrow()", function () {
    ...
    it("Should fail in beforeBorrow() if token price is unavailable in controllerStock", async function () {
      const {
        controllerStock,
        iToken0,
        mockPriceOracle,
  ...
  describe("beforeTransfer()/afterTransfer()", function () {
    ...
    it("Should fail in beforeTransfer() if account's any asset price is invalid in controllerStock", async function () {
      const {
        controllerStock,
        iToken0,
        iToken1,
      ...
      // Use controllerStock
      await iToken0._setController(controllerStock.address);
      await iToken1._setController(controllerStock.address);
      ...
      await controllerStock.connect(account1).enterMarkets([iToken0.address]);
      await iToken1.connect(account1).borrow(mintiToken1Amount.div(10));
