...
async function deployControllerStock(proxyAdminAddress) {
  const Controller = await ethers.getContractFactory("ControllerStock");

  const controllerStock = await deployProxy(Controller, [], {
    unsafeAllowCustomTypes: true,
    initializer: "initialize",
  });
...
async function fixtureiToken([wallet, other], provider) {
  ...
  const { controller, controllerStock, iToken0, iToken1, iToken6, iETH } = results;
...
async function fixtureDefault([wallet, other], provider) {
  const results = await loadFixture(fixtureEUR);
  const { controller, controllerStock, interestRateModel, xUSD, xEUR } = results;
