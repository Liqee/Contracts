...
describe("Governance Proposal Examination", function () {
  ...
  describe("Test goverance contract", function () {
    before(
      "Transfer controller contract owner to timelock contract",
      async function () {
        await init();
        ...
        await timelock
          .connect(user1)
          .executeTransaction(controller.address, 0, "_acceptOwner()", "0x");
    ...
    it("Transfer controller contract ownership from timelock contract to a new account", async function () {
      console.log("controller original owner is: ", await controller.owner());
      let data = abiCoder.encode(["address"], [user2.address]);
      await timelock
        .connect(user1)
        .executeTransaction(
          controller.address,
