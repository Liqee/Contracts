...
async function main() {
    ...
    let msdImplementation = commonConfigs.msdImplementation;
    if (!msdImplementation) {
        console.log("Do not get a valid MSD implementation contract address!");
        return;
    }

    // deploy MSD tokens.
    for (index in assetsToDeploy) {
        let msdToken = (assetsToDeploy[index]).toLowerCase();
        ...
        if (!msdTokenAddress) {
            ...
            let MSD = await deployMSD(assetConfig.msdTokenName, assetConfig.msdTokenSymbol, assetConfig.decimals, msdImplementation);
