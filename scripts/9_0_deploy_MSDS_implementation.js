...
const assetsToDeploy = ['xUSD'];

let deployer;

async function deployMSDSContract(assetConfig, asset, msdControllerAddress, fixedInterestModel, msdsAddress, commonConfigs) {
...
async function main() {
    ...
    for (index in assetsToDeploy) {
        ...
        let msdsToken = await deployMSDSContract(assetConfig, asset, msdController.address, fixedInterestModel, msdsAddress, commonConfigs);
